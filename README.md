...
- Windows 与 macOS 编辑工具栏均可一键插入 `&emsp;&emsp;` 和 `<br><br>`；快捷键分别为 `Alt+1`、`Alt+2` 与 `⌥1`、`⌥2`
- 编辑时输入 `<` 会出现受控的安全候选，可插入 `<br>`、`<br><br>`、黄色高光和红色文字格式
- Windows 提供 Markdown 原文、纯文本和富文本三种整篇复制；富文本可粘贴到 Word 等应用并保留排版
- Windows 使用 `Ctrl+F`、macOS 使用 `⌘F` 查找当前文档文字，并高亮全部结果；两端保留各自的系统全屏操作
- Windows 提供可收起、可调宽度的 Codex 助读侧栏，可针对整篇文章或选中文字连续提问；回答方式可选“只依据原文 / 自然回答 / 联网查证”
- macOS 可原生打开 PNG、JPG/JPEG、WebP、GIF、BMP，并提供图片缩放、适合窗口、原始大小、滚轮缩放与拖动查看
- macOS 会记住上一次图片缩放模式和百分比；切换图片时保留缩放习惯并从图片顶部开始
- 打开单个 `.md` / `.markdown` 或支持的本地图片，也可以打开整个文件夹
- 自动列出同文件夹第一层 Markdown 与图片，分别按文件名自然排序并连续翻阅
- macOS 左侧标签统一为“图片 / 文档 / 本文目录”；目录按标题层级缩进，点击即可在阅读或编辑位置跳转
...
- macOS 图片阅读直接使用系统 `NSImage` 从本地文件解码，不上传图片，也不把图片发送给 Codex
- 外部链接交给系统默认浏览器打开
