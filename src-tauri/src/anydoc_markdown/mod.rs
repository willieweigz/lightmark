//! GitHub-Flavored Markdown serializer for the AnyDoc document model.
//!
//! Derived from AnyDoc 0.1.8 under the MIT License. LightMark's only
//! behavioral change is resolving embedded `AssetId` images to local relative
//! paths so they remain at their original reading position.
...
use anchors::{resolve_anchors, AnchorMap};
use anydoc::model::{
    inlines_are_empty, AssetId, Block, Document, Inline, List, MarkerKind, Note, TableKind,
};
use escape::{backtick_fence, escape_text, EscapeOpts, InlineContext};
...
pub fn document_to_markdown(doc: &Document, asset_urls: HashMap<AssetId, String>) -> String {
    let rc = Ctx {
...
fn render_block(block: &Block, rc: &Ctx) -> Option<String> {
    match block {
        ...
        Block::Table(t) if t.kind == TableKind::Layout && t.is_single_cell() => {
            let anydoc::model::CellSlot::Origin(cell) = &t.grid[0][0] else {
                unreachable!()
            };
