...
use crate::anydoc_markdown::escape::InlineContext;
use crate::anydoc_markdown::inline::render_inlines;
use crate::anydoc_markdown::Ctx;
use anydoc::model::{Block, Cell, CellSlot, Table};
    ...
    match block {
        ...
        Block::List(list) => {
            for (i, item) in list.items.iter().enumerate() {
                ...
                let marker = match (&item.marker_label, list.marker) {
                    (Some(label), _) => format!(
                        "{} ",
                        crate::anydoc_markdown::escape_marker_label(
                            label,
                            InlineContext::TableCell
                        )
                    ),
                    (None, anydoc::model::MarkerKind::Bullet) => "• ".to_string(),
                    (None, kind) => {
        ...
        Block::CodeBlock { text, .. } => {
            ...
            if !t.is_empty() {
                ...
                crate::anydoc_markdown::inline::push_code_span(t, &mut s);
                parts.push(s);
            }
        }
        Block::Rule => {}
