...
use crate::anydoc_markdown::escape::{
    backtick_fence, escape_text, escape_url_as_text, format_url, EscapeOpts, InlineContext,
};
use crate::anydoc_markdown::Ctx;
use anydoc::model::{inlines_are_empty, ImageSource, Inline, LinkTarget, Style};
use std::borrow::Cow;
...
fn render_image(
    ...
    match source {
        ...
        ImageSource::Asset(id) if rc.asset_urls.contains_key(id) => {
            let label = if alt.trim().is_empty() {
                format!("导入图片 {}", id.0 + 1)
            } else {
                alt.trim().to_owned()
            };
            let label = escape_text(
                &label,
                ctx,
                EscapeOpts {
                    in_label: true,
            ...
            let _ = write!(
                ...
                "![{}]({})",
                label,
                format_url(rc.asset_urls.get(id).expect("checked asset URL"))
            );
        }
        // Unsupported embedded objects and missing images retain their alt
        // text without creating an unsafe or broken local reference.
